...
fun JellyBlob(
    ...
    Canvas(modifier.size(size)) {
        ...
        val shadowBlurPaint = createBlurPaint(
            color = color.copy(alpha = shadowAlpha),
            radius = baseR * shadowBlurFactor * shadowSpread
        )

        drawIntoCanvas { canvas ->
            canvas.drawCircle(
                Offset(cx, cy + baseR * 0.1f + shadowLift),
                baseR * shadowSpread,
                shadowBlurPaint
            )
        }
